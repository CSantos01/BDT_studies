...
def optimize_and_evaluate(classifier_name):
    ...
    study.optimize(
        lambda trial: objective(trial, classifier_name), n_trials=20
    )
    ...
    return classifier_name, evaluate_classifier(
...
with concurrent.futures.ThreadPoolExecutor() as executor:
    future_to_classifier = {
        executor.submit(optimize_and_evaluate, clf): clf for clf in classifiers
    }
    for future in concurrent.futures.as_completed(future_to_classifier):
        ...
        joblib.dump(
            best_model,
            __model_dir__ / f"{classifier_name}_best_model{extra_label}.pkl",
        )
...
def plot_metrics_log_scale(results, metric_name, ylabel):
    ...
    plt.tight_layout()
    plt.savefig(
        __output_dir__ / f"{metric_name}_comparison_log_scale{extra_label}.pdf"
    )
    plt.close()
